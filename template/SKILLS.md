...
| `update-harness` | Met à jour ou réinstalle ce harness Cerberus via la CLI (update / reinstall). Template curé Nardjo/cerberus uniquement — pas une sync mattpocock. À utiliser pour "update harness", "reinstall harness", "mets à jour mon harness". |
| `install-skill` | Installe ou adopte un Agent Skill dans ce harness Cerberus (skills/), jamais seulement dans les dossiers des outils. À utiliser pour "install skill", "add skill", api2cli, skills.sh, skill GitHub/cloud, ou une skill déjà posée sous ~/.claude/skills etc. |
